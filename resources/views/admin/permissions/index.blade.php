...
<div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Danh sách Quyền</h2>
                <p class="text-sm text-gray-500 mt-1">Quản lý tất cả quyền hạn trong hệ thống</p>
            </div>
            <a href="{{ route('admin.permissions.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
    ...
    <!-- Filters and Search -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <form method="GET" action="{{ route('admin.permissions.index') }}" class="flex flex-col lg:flex-row gap-4">
            <!-- Search -->
            <div class="flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" 
                           ...
                           placeholder="Tìm kiếm theo tên, slug, nhóm hoặc mô tả..." 
            ...
            @if($groups->count() > 0)
            <div class="lg:w-48">
                <label for="group" class="sr-only">Nhóm</label>
                <select id="group" 
                        name="group" 
            ...
            <div class="lg:w-40">
                ...
                <select id="sort_by" 
                        name="sort_by" 
                        class="block w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                    <option value="group" {{ request('sort_by') === 'group' ? 'selected' : '' }}>Nhóm</option>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.permissions.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors">
                    Reset
                </a>
            </div>
        </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <div>
                                ...
                                <div class="text-sm text-gray-500 mt-1">{{ \Illuminate\Support\Str::limit($permission->description, 50) }}</div>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <code class="text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded">{{ $permission->slug }}</code>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($permission->group)
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">Không có quyền nào</h3>
                            <p class="mt-1 text-sm text-gray-500">Hãy thêm quyền đầu tiên để bắt đầu.</p>
                            <div class="mt-6">
                                <a href="{{ route('admin.permissions.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex items-center space-x-1">
                    {{ $permissions->appends(request()->query())->links('pagination::simple-tailwind') }}
                </div>
            </div>
        </div>
        @endif
